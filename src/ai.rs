...
//if piece == spawn location then calc board and set a list of desired moves and coords             MAYBE RUN ON EACH UPDATE INSTEAD OF ONLY SPAWN IF FAST ENOUGH     IF AI CAN HANDLE 60FPS THEN YES      WHAT ABOUT AFTER PIECE SET WHEN PIECE SPAWNED BUT BOARD NOT UPDATED? WHEN UPDATED IT DROPS PIECE
...
//weight of each parameter val is a 0-1 float. add together to get move val         USE GENERATIONAL ALG TO TEST ON VERSION WITH NO DELAY
...
//  find best location
//  generate moves to get to coords, rotation, held piece
